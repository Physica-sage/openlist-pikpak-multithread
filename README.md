...
批量目录并行只覆盖本补丁接管的**同存储批量移动、复制和合并**，不会改变手动 STRM 扫描。调度前会清洗并去重路径；同一存储中只要出现相同路径、父子路径或根目录重叠，整批会保守地使用上游原始串行递归。互不重叠时使用公平的目录级队列：初始顶层目录排在递归发现的子目录之前，避免慢速深层目录占满 worker 后饿死剩余顶层目录。OpenList 的 `Handle hook rate limit` 大于 `0` 时所有 worker 共用同一个 limiter，既保留配置并发，也不会成倍放大目录 List 请求速率。
